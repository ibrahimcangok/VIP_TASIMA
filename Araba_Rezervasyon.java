...
public class Araba_Rezervasyon { //Inheritence(kalıtım) : extends kullandigimiz yerler kalitima giriyor.
    final String[] gunler = {"Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi", "Pazar"};
    private final List<String> gunlerList = Arrays.asList(gunler);  //ArrayList'i kullandik.
    //ArrayList sınıfı öğeleri depolamak için dinamik bir dizi kullanır.
    // Normal bir dizi gibidir fakat boyut sınırı yoktur. İstenildiği zaman öğe eklenir veya öğe silinebilir.
    static int toplamFiyat;
    final int normalSedanFiyat = 300, vipSedanFiyat = 450, normalMinivanFiyat = 400, vipMinivanFiyat = 600;
    public static int getToplamFiyat() {return toplamFiyat;} //Getter:Encapsulation'da nesneye direkt ulasmamizi engelleyen bir metod'dur.
    public int getNormalSedanFiyat() {return normalSedanFiyat;} //Getter ile değerleri metod vasitasiyla nesneye atariz.
    public int getVipSedanFiyat() {return vipSedanFiyat;} //Setter methodunu kullanmadık çünkü hemen hemen bütün degerleri static olarak tanımladık.
    public List<String> getGunlerList() {return gunlerList;} //Degiskenlerimiz final tipten olduğu için Encapsulation kullanmadık.
    ...
    //False olmasının sebebi başlangıçta hiçbi rezervasyonun olmamasıdır yani 0'dır.
    //Static tanımlamamızın sebebi
    //Static degiskenlere nesne üretmeden ulasabilir cünkü sinifta olan bütün static degerler pointer vasitasiyla bellekte belli bir yerde saklanir ve
    //onlara sinif ismi ile ulasabiliriz
...
public static void rezervasyonIptal(String gun, int aracNo) {
        Araba_Rezervasyon rezervasyon = new Araba_Rezervasyon();
        if (aracNo == 1) {
            if (gun.equalsIgnoreCase(rezervasyon.getGunlerList().get(0))) {
                if (pazartesi1) {
    ...
    @Override  //Polymorphism : Ana sinifta olan toString fonksiyonunu alt sinifimizda override yaparak Polymorphismi gerceklestirdik.
    public String toString() { //Simdi ana sinifta farkli bir fonksiyonumuz var - alt sinifimizda da farkli bir fonksiyonumuz var.
